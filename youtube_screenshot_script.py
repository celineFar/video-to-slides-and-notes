...
# The newer one: should be replaced
def download_video(
    url: str,
    output_dir: str,
...
def run_extraction(
    ...
    if blur < 0 or blur > 1000:
        raise ValueError("Blur threshold must be between 0 and 1000.")
    
    if watermark_threshold < 0 or watermark_threshold > 1:
    ...
    if use_gpu:
        try:
            ...
            print("Warning: PyCUDA not installed. Falling back to CPU.")
            use_gpu = False
    ...
    if source.startswith(('http://', 'https://', 'www.')):
        video_path = f"downloaded_video_{timestamp}.mp4"
        video_title = download_video(source, video_path, max_resolution, verbose)
        sanitized_title = sanitize_filename(video_title)
    else:
        video_path = source
        video_title = os.path.splitext(os.path.basename(video_path))[0]
    ...
    if not dry_run:
        start_time = time.time()
        total, skipped, saved = extract_frames(
            video_path, output_folder, method, interval, quality, blur,
            detect_watermarks, watermark_threshold, use_parallel, png, use_gpu,
            fast_scene, resume, verbose, gradfun, deblock, deband
        )
        end_time = time.time()
        print(f"\nDone! Processed: {total}, Saved: {saved}, Skipped: {skipped}")
